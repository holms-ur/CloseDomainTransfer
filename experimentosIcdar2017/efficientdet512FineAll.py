...
train_tfms = tfms.A.Adapter([*tfms.A.aug_tfms(size=(size,size), presize=presize,shift_scale_rotate=None,crop_fn=None, horizontal_flip=None, pad=None), tfms.A.Normalize()])

train_ds = Dataset(train_records, train_tfms)
valid_ds = Dataset(valid_records, valid_tfms)

model=efficientdet.model(model_name='tf_efficientdet_d2', num_classes=4,img_size=size)
model.model.reset_head(num_classes=6)

fastai.load_model('../models/ice-faster-efficientdetFine-v1.pth',model,None,with_opt=False)
model.model.reset_head(num_classes=len(class_map))
...
saveM=SaveModelMAP(monitor='COCOMetric',fname='model-ice-efficientdetIcdarFineAll-v1')
...
learn.save('ice-efficientdetIcdarFineAll-v1')
